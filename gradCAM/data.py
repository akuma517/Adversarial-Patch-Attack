...
def initialize_data(folder):
    zip_folder = folder + '/gtsrb-german-traffic-sign.zip'
    if not os.path.exists(zip_folder) :
        raise(RuntimeError("Could not find " + zip_folder + 
              + ', please download them from https://www.kaggle.com/c/nyu-cv-fall-2017/data '))
              
    # extract train_data.zip to train_data
    train_folder = folder + '/train'
    if not os.path.isdir(train_folder):
        print(train_folder + ' not found, extracting ' + zip_folder)
    ...
    if not os.path.isdir(test_folder):
        print(test_folder + ' not found, extracting ' + zip_folder)
        zip_ref = zipfile.ZipFile(zip_folder, 'r')
        zip_ref.extractall(folder)
        zip_ref.close()
